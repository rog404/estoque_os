...
Phoenix LiveView behind email+password auth, Postgres in Docker on a
**non-default port**. Both bite if you assume the usual setup.

## Start it

```bash
docker compose ps                 # postgres must be "Up (healthy)" — host port 5433, NOT 5432
...
New accounts are created with `must_reset_password: true`, so the first login
is forced through a password change even when you passed `--password`. Re-running
the task on an existing email only changes its role; it does not clear the flag.

Roles are `admin`, `manager`, `marketing`, `logistics`, `auditor` — and they see
genuinely different screens. `logistics` cannot see money; `auditor` can see
...
Logging in over `curl` fights LiveView's CSRF and is not worth the time. Render
the page through the app's own test stack and screenshot the HTML — real
template, real compiled CSS, only the transport differs.
...
defmodule EstoqueOSWeb.ScreenshotDumpTest do
  ...
  setup :register_and_log_in_operator   # manager; also _admin, _logistics, or _as(ctx, role)
...
`light` is the default theme; `dark` exists for badly lit mission storage rooms.
Headless chromium picks dark on its own, so set `data-theme` explicitly or you
will review the wrong one.

## Resetting

```bash
mix ecto.reset      # drop + create + migrate + seeds.exs (locations, products, kits from priv/samples/)
