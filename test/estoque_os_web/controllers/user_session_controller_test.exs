defmodule EstoqueOSWeb.UserSessionControllerTest do
  ...
  describe "POST /users/log-in - email and password" do
    ...
    test "logs the user in with remember me", %{conn: conn, user: user} do
      ...
      assert conn.resp_cookies["_estoque_os_web_user_remember_me"]
      assert redirected_to(conn) == ~p"/"
    end
  ...
  describe "repeated failures" do
    test "start costing time", %{conn: conn} do
      user = user_fixture()

      conn =
        Enum.reduce(1..4, conn, fn _, acc ->
          post(acc, ~p"/users/log-in", %{
    ...
    test "the wait applies even to the right password", %{conn: conn} do
      user = user_fixture()

      conn =
        Enum.reduce(1..3, conn, fn _, acc ->
          post(acc, ~p"/users/log-in", %{
            "user" => %{"email" => user.email, "password" => "wrong wrong wrong"}
          })
        end)

      # Checking the password first would leak, through timing, whether the guess
      # was right. The wait is answered before anything is checked.
      conn =
        post(conn, ~p"/users/log-in", %{
          "user" => %{"email" => user.email, "password" => valid_user_password()}
        })
